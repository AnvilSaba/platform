...
Bot と MCGuildLink は独立してバージョン管理します。タグはそれぞれ `bot/vX.Y.Z`、`mcguildlink/vX.Y.Z` です。変更履歴は各アプリの `CHANGELOG.md` に生成されます。
...
アプリ固有の変更だけが対象です。Bot では `apps/bot`、`crates/bot-macros` と Rust のルートマニフェスト、MCGuildLink では `apps/mcguildlink` 以下を追跡します。
...
2. アプリ固有の基準コミット以降から、バージョンファイルと完全な `CHANGELOG.md` を再生成する
...
ブランチ保護で GitHub Actions から `main` への直接 push を禁止している場合は、Repository settings でこのWorkflowを許可する必要があります。
...
リリース済みの履歴と現在の未リリース変更をまとめて再生成します。Botは`59d55a6`より後、MCGuildLinkは`ead3cf1`より後だけが対象です。
...
生成した変更ログの末尾には、対象外にした過去のコミット履歴へのリンクが`v3.5.0以前`または`v1.0.0以前`として追加されます。

アプリごとのバージョンファイル、変更ログ出力先、対象パス、基準コミットと基準タグは`scripts/release-config.psd1`で一元管理します。
